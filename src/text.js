...
class TestReportGenerator {
   ...
   * Generates a textual summary of test results, including checks, metrics, thresholds, groups, and scenarios.
   *
   * @param {Report} report - The report object containing thresholds, checks, metrics, groups,
   * and scenarios as provided by k6.
   * @returns {string} - A formatted summary of the test results.
   */
  generate(report) {
...
class ReportBuilder {
  ...
  _renderMetrics(metrics, renderContext) {
    ...
    return Object.entries(metrics)
      .filter(([_, sectionMetrics]) => Object.keys(sectionMetrics).length > 0)
      .reduce(
      ...
      .flatMap(([sectionName, sectionMetrics]) => [
        renderContext.indent(this.formatter.boldify(sectionName.toUpperCase())),
        ...renderMetrics(
          sectionMetrics,
          summaryInfo,
          this.formatter,
...
class ANSIFormatter {
  ...
  decorate(text, color, ...styles) {
    ...
    const styleCodes = styles.map((style) => ANSIStyles[style]).filter(Boolean);
 ...
 * @param {SummaryInfo} summaryInfo - An object containing summary information such as
 * maximum name width and trend columns.
...
function renderThresholds(metrics, formatter, renderContext, options) {
  ...
  // Precompute all formatting information
  const summaryInfo = computeSummaryInfo(metrics, renderContext, options);
  ...
  for (const name of metricNames) {
    ...
    const innerContext =
      isSubmetric && parentMetricExists
 ...
 */
function renderThresholdResults(metric, summaryInfo, formatter, renderContext) {
  const lines = [];
 ...
 */
function renderMetricValueForThresholds(metric, threshold, info, formatter) {
  const { trendStats, trendCols, trendKeys, nonTrendValues, nonTrendExtras } =
    info;
  ...
  switch (metric.type) {
    ...
    case 'rate':
      return [
        ...
        `${metric.values.passes} out of ${
          metric.values.passes + metric.values.fails
        }`,
      ];
    default:
      return ['[no data]'];
